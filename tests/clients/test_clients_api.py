...
@pytest.mark.django_db
def test_filter_by_documento(api_client, clientes):
    list_url = reverse("cliente-list")

    response = api_client.get(list_url, {"documento": "123"})
...
@pytest.mark.django_db
def test_filter_by_ids(api_client, clientes):
    c1, c2 = clientes
    list_url = reverse("cliente-list")

    response = api_client.get(list_url, {"ids": f"{c1.id}"})
...
@pytest.mark.django_db
def test_filter_by_ids_empty(api_client, clientes):
    list_url = reverse("cliente-list")

    response = api_client.get(list_url, {"ids": ""})
...
@pytest.mark.django_db
def test_cliente_deudas_post_invalid(api_client, clientes):
    cliente = clientes[0]
...
@pytest.mark.django_db
def test_cliente_deudas_get(api_client, cliente, deuda):
    url = reverse("cliente-deudas", args=[cliente.id])
...
@pytest.mark.django_db
def test_cliente_deudas_post_valid(api_client, cliente):
    url = reverse("cliente-deudas", args=[cliente.id])
...
@pytest.mark.django_db
def test_cliente_portafolios(api_client, cliente, portafolio):
